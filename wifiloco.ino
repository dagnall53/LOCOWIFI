 ...
 // WriteSV2EPROM();   ///  comment out later....use if needed to reset eprom
  ...
  //EEPROM.commit();
    if((EEPROM.read(255) == 0xFF) && (EEPROM.read(256) == 0xFF)){ //eeprom empty, first run * comment outthis and 
     ...
     EPROM2SV();
   ShowSVS();  
   MyLocoAddr=CV[18]+((CV[17]-192)*256);
...
Serial.print(" My 'Locomotive Address' is");
Serial.print (CV[18]+((CV[17]-192)*256));
Serial.print(" CV17=");
  Serial.print(CV[17]); 
  Serial.print(" CV18=");
   Serial.print(CV[18]); 
 ...
 //periodic functions
  ...
  if(recLen){    // Check if a rocrail client has connected
        ...
        // show all messages for debug
         ...
         Serial.print(F("         Our 'Board address' starts at:"));
         Serial.print(FullBoardAddress); 
          Serial.print(F("   Our 'LocoAddr' is:"));
     ...
     Serial.println( ")  ");    
       ...
       if (recMessage[0]== OPC_PEER_XFER){
        ...
        if (((CommandFor(recMessage, recLen)== FullBoardAddress)  ||(OPCSRCL== 0))){  // my address or broadcast
         ...
         else {if (OPCSRCL== 0){ delay((SV[1]-80)*20);}}
